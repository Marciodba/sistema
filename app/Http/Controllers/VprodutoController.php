...
class VprodutoController extends Controller
{
    public function index()
    {
        ...
        $produtoprecos = ProdutoPreco::select(DB::RAW('produto.id AS produtoid, produto.codigo AS produtocodigo, produto.nome AS produtonome, 
        produto.ativo AS produtoativo, produto.dtcad AS produtodtcad, produto.idgruposbb AS produtoidgruposbb, produto.observacao AS produtoobservacao, 
        produto.estoqueminimo AS produtoestoqueminimo, produto.comprarqtde AS produtocomprarqtde, produto.dtatualizacao AS produtodtatualizacao, 
        produto.codigobarra AS produtocodigobarra, produto.idunidade AS produtoidunidade, produto.codigofabricante AS produtocodigofabricante,
        produto.idmodelo AS produtoidmodelo, produto.dtlixo AS produtodtlixo, 
       ...
       produto.tempopreparo as produtotempopreparo,
        produtogruposbb.id AS produtogruposbbid, produtogruposbb.idprodutogruposub AS produtogruposbbidprodutogruposub,
        produtogruposbb.codigo AS produtogruposbbcodigo, produtogruposbb.nome AS produtogruposbbnome, produtogruposbb.km AS produtogruposbbkm,
        produtogruposbb.dtlixo AS produtogruposbbdtlixo, produtogruposbb.dtatualizacao AS produtogruposbbdtatualizacao,
       ...
       produtopreco.qtde as produtoprecoqtde 
  '))->Join('produto', 'produto.id', '=', 'produtopreco.idproduto')
        ->Join('produtogruposbb', 'produtogruposbb.id', '=', 'produto.idgruposbb')
      ->where('produto.id', '>', 0)->limit(50)->get();
