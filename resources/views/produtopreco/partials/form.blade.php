    ...
    <form method="get" class="signup-form">
        ...
        <div class="flex gap-4">
            <x-bladewind.input name="codigo" required="true" label="Codigo"
                value="{{ $produtopreco[0]->produtocodigo ?? old('codigo') }}" error_message="codigo Obritatoria" />

            <x-bladewind.input name="nome" required="false" label="Descrição"
                value="{{ $produtopreco[0]->produtonome ?? old('nome') }}" error_message="Descriçao Obritatoria" />

            <x-bladewind.input name="codigobarra" required="false" label="codigobarra"
                value="{{ $produtopreco[0]->produtocodigobarra ?? old('codigobarra') }}"
                error_message="Descriçao Obritatoria" />
        </div>
     
  

            <x-bladewind.select name="classificacao" required="true"
            placeholder="Classificação"
            selected_value="{{$produtopreco[0]->produtoidgruposbb }}"
            ...
            <x-bladewind.input name="observacao" required="false" label="observacao"
                value="{{ $produtopreco[0]->produtoobservacao ?? old('observacao') }}" error_message="Descriçao Obritatoria" />

            <x-bladewind.input name="preco" required="false" label="preco" placeholder="0.00" prefix="R$"
                numeric="true" value="{{ $produtopreco[0]->produtoprecopreco ?? old('preco') }}"
                error_message="Descriçao Obritatoria" />
